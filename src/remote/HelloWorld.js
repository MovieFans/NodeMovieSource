...
HelloWorldClient.prototype.sayHello = function(username, callback) {
...
HelloWorldClient.prototype.send_sayHello = function(username) {
...
HelloWorldClient.prototype.recv_sayHello = function(input,mtype,rseqid) {
...
HelloWorldProcessor.prototype.process_sayHello = function(seqid, input, output) {
  var args = new HelloWorld_sayHello_args();
  args.read(input);
  input.readMessageEnd();
  if (this._handler.sayHello.length === 1) {
    Q.fcall(this._handler.sayHello, args.username)
      .then(function(result) {
        var result = new HelloWorld_sayHello_result({success: result});
    ...
    this._handler.sayHello(args.username, function (err, result) {
      if (err == null) {
        var result = new HelloWorld_sayHello_result((err != null ? err : {success: result}));
        output.writeMessageBegin("sayHello", Thrift.MessageType.REPLY, seqid);
      } else {
        var result = new Thrift.TApplicationException(Thrift.TApplicationExceptionType.UNKNOWN, err.message);
        output.writeMessageBegin("sayHello", Thrift.MessageType.EXCEPTION, seqid);
